...
const connector = connect(mapStateToProps, {setAlert, getRecipes, deleteRecipe});
...
const Recipes: React.FC<Props> = ({showModal, setShowModal, getRecipes, deleteRecipe, _loading, user, recipes, setNavigate, setAlert, isAuthenticated}) => {
  ...
  const [pageResults, setPageResults] = useState<Recipe[]>([]);
  ...
  const filterByPage = (rec: Recipe, i: number) => {
  ...
  const filterResults = () => {
    ...
    const arr = recipes.filter(res => {
      ...
      if (type === 'category') {
        ...
        const bool = calories.total < showModal.Filter.filter!;
        returnValue = bool ? res : null;
      }
