...
const Nutrients: React.FC<Props> = ({recipe, scale}) => {
    const initNuts = {
        iron: {
            amount: 0,
            unit: ''
    ...
    useEffect(() => {
        if (!recipe[0]?.nutrients) return;
        const temp = {...recipe[0].nutrients.total};
        setNutrients(temp);
        setInitAmounts(recipe[0].nutrients.total);
        setCalories(Number(recipe[0].calories.total));
        setInitCalories(Number(recipe[0].calories.total));
    }, [recipe[0]]);
    useEffect(() => {
